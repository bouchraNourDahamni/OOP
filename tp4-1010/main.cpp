...
int main()
{
	...
	// 1. le constructeur par défaut de client doit fonctionner
	...
	// 2. le constructeur par paramètres de client doit fonctionner
	...
	// 3. le constructeur par défaut de client premium doit fonctionner
	...
	// 4. le constructeur par paramètres de client premium doit fonctionner
	...
	// 5. le constructeur par défaut de fournisseur doit fonctionner
	tests.push_back(mina.obtenirNom() == "Doe" &&
		mina.obtenirPrenom() == "John" &&
		mina.obtenirIdentifiant() == 0 &&
		mina.obtenirCodePostal() == "A1A A1A");
	// 6. le constructeur par paramètres de fournisseur doit fonctionner
	...
	// 10. le constructeur par paramètres de produit aux enchères doit fonctionner
	...
	// 11. le constructeur par défaut de produit soldé doit fonctionner
	...
	// 12. le constructeur par paramètres de produit soldé doit fonctionner
	...
	// 13 à 15. une surenchère doit être strictement supérieure à l'offre précédente
	...
	// 24. on ne peut pas ajouter le même produit deux fois dans le panier
	gaspard.ajouterProduit(&stylo); // produit normal
	gaspard.ajouterProduit(&nem);   // produit soldé
	tests.push_back(gaspard.obtenirPanier().size() == 2);
	// 25. idem pour client premium
	julie.ajouterProduit(&stylo); // produit normal
	julie.ajouterProduit(&nem);   // produit soldé
	tests.push_back(julie.obtenirPanier().size() == 3);

	// 26 à 27. ajouter le produit au fournisseur doit l'ajouter au catalogue
	...
	tests.push_back(mina.obtenirCatalogue().size() == 1 &&
		mina.obtenirCatalogue()[0]->obtenirNom() == "montre"); // soldé
															   // 28. modifier le fournisseur d'un produit doit ajouter le produit au nouveau fournisseur
	...
	// 29. modifier le fournisseur d'un produit doit retirer le produit du catalogue précédent
	tests.push_back(mina.obtenirCatalogue().size() == 0);
	// 30. enlever un produit doit mettre l'attribut fournisseur à jour
	martine.enleverProduit(&montre);
	tests.push_back(montre.obtenirFournisseur() == nullptr);
	// 31. enlever un produit doit le retirer du catalogue du fournisseur
	tests.push_back(martine.obtenirCatalogue().size() == 2 &&
		martine.obtenirCatalogue()[0]->obtenirNom() == "chaussures" &&
		martine.obtenirCatalogue()[1]->obtenirNom() == "violoncelle");
	// 32. enlever un produit une deuxième fois ne doit rien faire
	martine.enleverProduit(&montre);
	tests.push_back(martine.obtenirCatalogue().size() == 2 &&
		martine.obtenirCatalogue()[0]->obtenirNom() == "chaussures" &&
		martine.obtenirCatalogue()[1]->obtenirNom() == "violoncelle");

	// 33. réinitialiser doit vider le panier de tous les clients
	poly.reinitialiser();
	...
	// 34. réinitialiser doit mettre à jour l'attribut encherisseur_ quand nécessaire
	tests.push_back(violoncelle.obtenirEncherisseur() == nullptr);
	// 35. réinitialiser doit remettre les produits aux enchères à leur prix intial
	tests.push_back(violoncelle.obtenirPrix() == violoncelle.obtenirPrixInitial());
	// 36. réinitialiser doit vider le catalogue de tous les fournisseurs
	tests.push_back(martine.obtenirCatalogue().size() == 0 &&
		samuel.obtenirCatalogue().size() == 0 &&
		mina.obtenirCatalogue().size() == 0);
	// 37. réinitialiser doit mettre à jour l'attribut fournisseur_ quand nécessaire
	tests.push_back(stylo.obtenirFournisseur() == nullptr &&
		sculpture.obtenirFournisseur() == nullptr &&
		nem.obtenirFournisseur() == nullptr);

	// 38 à 39. le chiffre d'affaires devrait être nul après une réinitialisation
	...
	// 40. le total à payer des clients premiums doit prendre ne compte la réduction
	...
	// 41. la réduction ne doit pas induire des prix négatifs
	julie.ajouterProduit(&nem); // 0
	tests.push_back(julie.obtenirTotalAPayer() == 21095.0);
	// 42. la surenchère doit actualiser le prix à payer pour l'ancien encherisseur
	tests.push_back(gaspard.obtenirTotalAPayer() == 110.0);
	// 43. le total à payer des fournisseurs doit être nul
	samuel.ajouterProduit(&chaussures);
	samuel.ajouterProduit(&montre);
	tests.push_back(samuel.obtenirTotalAPayer() == 0.0);
	// 44. le chiffre d'affaires doit additionner les totaux à payer
	tests.push_back(poly.obtenirChiffreAffaires() == 110.0 + 21095.0);

	// 45 à 46. un usager ne peut pas être ajouté deux fois au gestionnaire
